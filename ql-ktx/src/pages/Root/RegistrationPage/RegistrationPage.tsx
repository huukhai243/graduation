...
const RegistrationsPage = () => {
  ...
  const convertRoom = (roomNumber: number) => {
    const room = rooms.find((room) => room.Roomnumber === roomNumber)
    return room ? `${room.Building.name} - ${room.name}` : ''
  }
  ...
  return (
    <div className="flex w-full h-screen justify-center items-center">
      <form onSubmit={onSubmit} className="space-y-8 border p-4 w-3/4">
        ...
        <div className="p-3">
          <div>Nguyện vọng 1: {isSuccess && convertRoom(checkedRooms[0])}</div>
          <div>Nguyện vọng 2: {isSuccess && convertRoom(checkedRooms[1])}</div>
          <div>Nguyện vọng 3: {isSuccess && convertRoom(checkedRooms[2])}</div>
        </div>
